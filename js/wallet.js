...
function switchWalletTab(tab) {
  ...
  const names = ['cashin', 'cashout', 'demo', 'promo', 'history'];
  ...
  const map = { cashin: 'tabCashin', cashout: 'tabCashout', demo: 'tabDemo', promo: 'tabPromo', history: 'tabHistory' };
...
async function submitDeposit() {
  ...
  if (result.success) {
    ...
    Toast.show('📤', 'Deposit Submitted!', `₱${pending.amount.toLocaleString()} deposit is being verified. Credits will be added once approved.`, 'success', 6000);
    SFX.play('deposit');
    closeDepositModal();
  } else {
    submitBtn.disabled = false;
...
function redeemPromoCode() {
  ...
  const alreadyRedeemed = redemptions.find(r => r.code === code && r.userId === user.id);
  if (alreadyRedeemed) {
  ...
  const txns = JSON.parse(localStorage.getItem('scatterx_txns_' + user.id) || '[]');
  txns.push({
    type: 'promo',
    amount: amount,
    balanceType: promo.type,
    status: 'completed',
    method: 'Promo: ' + code,
    date: new Date().toISOString()
  });
  localStorage.setItem('scatterx_txns_' + user.id, JSON.stringify(txns));
