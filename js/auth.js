...
async function approveDeposit(userId, depositId, amount) {
  ...
  if (typeof CloudDB !== 'undefined' && CloudDB.available()) {
    const cloudSaved = await CloudDB.saveUser(userId, user);
    if (cloudSaved) {
      console.log('[Admin] ✅ Approved deposit — cloud balance updated for', userId);
    } else {
      console.error('[Admin] ❌ Cloud save FAILED for', userId, '— balance may not persist');
      Toast.show('⚠️', 'Cloud Save Failed', 'Balance added locally but cloud sync failed. Try again.', 'error', 5000);
    }
    if (CloudDB.deleteDeposit) await CloudDB.deleteDeposit(depositId);
  }
