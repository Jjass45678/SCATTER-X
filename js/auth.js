...
const Auth = (() => {
  ...
  function login(identifier, password) {
    ...
    const user = Object.values(users).find(u => u.identifier === normalizedId);
    if (!user) {
  ...
  <div class="modal" style="max-width:560px;padding:24px 28px">
    ...
    <div id="adminContent" style="display:none">
      <div style="display:flex;gap:6px;margin-bottom:14px">
        <button class="btn-primary" id="adminTabDeposits" onclick="switchAdminTab('deposits')" style="flex:1;padding:8px;font-size:.75rem">📋 Deposits</button>
        <button class="btn-primary" id="adminTabPromo" onclick="switchAdminTab('promo')" style="flex:1;padding:8px;font-size:.75rem;opacity:.5">🎁 Promo Codes</button>
      </div>
...
function getAllPendingDeposits() {
  ...
  // Also check the standalone pending list
  try {
...
function approveDeposit(userId, depositId, amount) {
...
function rejectDeposit(userId, depositId) {
  const users = JSON.parse(localStorage.getItem('scatterx_users') || '{}');
  const user = users[userId];
  if (!user) return;
...
function switchAdminTab(tab) {
  const dBtn = document.getElementById('adminTabDeposits');
  const pBtn = document.getElementById('adminTabPromo');
  const dPanel = document.getElementById('adminDepositsPanel');
  const pPanel = document.getElementById('adminPromoPanel');
  if (!dBtn || !pBtn) return;

  if (tab === 'deposits') {
    dBtn.style.opacity = '1'; pBtn.style.opacity = '.5';
    dPanel.style.display = 'block'; pPanel.style.display = 'none';
    renderPendingDeposits();
  } else {
    dBtn.style.opacity = '.5'; pBtn.style.opacity = '1';
    dPanel.style.display = 'none'; pPanel.style.display = 'block';
...
function adminDeletePromoCode(id) {
  const codes = getPromoCodes();
...
function adminTogglePromoCode(id) {
  const codes = getPromoCodes();
