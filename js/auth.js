  ...
  <div class="modal" style="max-width:560px;padding:24px 28px">
    ...
    <div id="adminContent" style="display:none">
      <div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap">
        ...
        <button class="btn-primary" id="adminTabTopup" onclick="switchAdminTab('topup')" style="flex:1;padding:8px;font-size:.72rem;opacity:.5">💸 Top-Up</button>
...
function switchAdminTab(tab) {
  const tabs = ['deposits', 'topup', 'users', 'promo'];
  const btnMap = { deposits: 'adminTabDeposits', topup: 'adminTabTopup', users: 'adminTabUsers', promo: 'adminTabPromo' };
  const panelMap = { deposits: 'adminDepositsPanel', topup: 'adminTopupPanel', users: 'adminUsersPanel', promo: 'adminPromoPanel' };
...
async function adminLoadCloudUsers() {
  if (typeof CloudDB === 'undefined' || !CloudDB.available()) {
    Toast.show('⚠️', 'Cloud Offline', 'GitHub not configured. See js/db.js', 'error');
    return;
  }
...
async function adminDirectCashIn(userId) {
  ...
  if (typeof CloudDB !== 'undefined' && CloudDB.available()) {
    ...
    if (result.success) {
      ...
      Toast.show('☁️', 'Cloud Updated', 'Balance synced to cloud', 'success', 2000);
    }
  }
